...
The staged process-reuse work is documented in
[docs/PROCESS-AND-LIFECYCLE.md](./docs/PROCESS-AND-LIFECYCLE.md) and the full
[requirements plan](./docs/2026-08-18-lsp-bridge-process-reuse-debate-and-plan.md).
The contract is the target for the rollout; installer, plugin, and generated
Codex configuration paths are not considered aligned until the acceptance
checks in those documents pass.

- The bridge's own runtime uses an approved native `node.exe` directly. It does
  not use a bare `node`, `node.cmd`, another command shim, or `node_repl.exe`.
- This rule does not remove support for `.cmd` or `.bat` launchers used by a
  configured language server when that server requires one.
...
Typical explicit-diagnostics loop:

1. Codex edits a supported source file or explicitly requests diagnostics.
2. The bridge asks the matching local language server for diagnostics on the
   touched file.
...
- **Windows startup reliability.** The bridge runtime and the configured
  language-server launcher are separate concerns. The bridge runtime uses a
  directly resolved native `node.exe`; language-server launchers may still be
  `.cmd` or `.bat` wrappers when required. The bridge resolves local
  `node_modules/.bin`, `PATH`, and `PATHEXT`, rewrites common npm shims to direct
  Node entrypoints, and uses a safe `cmd.exe` wrapper only when needed for the
  language server.
...
- Read-only diagnostics with `status`, `timedOut`, `stale`, `sourceRevision`,
  and `configurationIssues` metadata. Empty, stale, or timed-out responses are
  qualified instead of being reported as a clean type check.
...
The MVP is intentionally narrow and ready for explicit local use. Automatic
diagnostics are a separate, user-controlled path and remain disabled during
the staged process and load baseline.
...
The examples below show the shape of the generated registrations. The staged
runtime contract requires the final bridge and hook launch commands to resolve
the approved native `node.exe` directly; the `--auto-update` path must preserve
that rule rather than using a package-manager shim as the bridge runtime. Treat
...
The installer and updater must converge on that direct-runtime contract before
the staged process baseline can be called complete.
...
- Process and load measurement is opt-in and local. It uses negative and
  ...
  `INCONCLUSIVE`, not evidence of a load improvement.
