...
describe("language config", () => {
  ...
  it("detects supported languages from file extensions", () => {
    ...
    expect(detectLanguageFromFile("src/module.mjs")).toBe("typescript");
    expect(detectLanguageFromFile("src/config.cjs")).toBe("typescript");
...
describe("file URI helpers", () => {
  ...
  it("canonicalizes percent-encoded Windows path characters to the opened-document URI", () => {
    expect(canonicalizeFileUri("file:///c%3A/Users/JackKenter%28Intern%29/project/src/a.ts")).toBe(
      "file:///C:/Users/JackKenter(Intern)/project/src/a.ts"
    );
  });
});
