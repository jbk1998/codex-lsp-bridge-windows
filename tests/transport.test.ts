...
describe("MCP dispatch", () => {
  ...
  it("keeps the default MCP service for a relative file when no root argument is provided", async () => {
    const defaultService = new CommandService(new EmptyProvider());
    const scopedService = new CommandService(new EmptyProvider("detached"));
    let scopedCalls = 0;
  ...
  it("selects a scoped workspace for an absolute file when no root argument is provided", async () => {
    const defaultService = new CommandService(new EmptyProvider());
    const scopedService = new CommandService(new EmptyProvider("auto-detected"));
    const outsideFile = path.resolve(path.parse(process.cwd()).root, "tmp", "skill-package", "scripts", "verify.py");
    let scopedCalls = 0;
    ...
    await expect(
      ...
      structuredContent: {
        total: 1,
        items: [{ file: "auto-detected/src/a.ts" }]
      }
    });
