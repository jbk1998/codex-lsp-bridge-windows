...
- It opens workspace files through a verified descriptor, rejects links and
  identity changes, and revalidates the descriptor before sending content to a
  language server.
...
- Malformed global or workspace configuration blocks the operation with a
  path-specific diagnostic instead of silently widening behavior.
- MCP lines, active requests, queued responses, directory scans, and outgoing
  LSP payloads have fixed upper bounds to contain local memory and traversal
