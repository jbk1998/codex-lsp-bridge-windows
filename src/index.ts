...
async function collectDirectoryDiagnostics(
  ...
  return {
    ...summary,
    status,
    ...summarizeConclusion(status, summary.total, summary.stale, summary.configurationIssues.length),
...
function mergeDiagnosticSummaries(summaries: Array<Awaited<ReturnType<WorkspaceCommandService["diagnostics"]>>>): DiagnosticSummary {
  ...
  const status: DiagnosticStatus = summaries.some((summary) => summary.status === "unavailable")
    ? "unavailable"
    : summaries.some((summary) => summary.status === "timed_out")
      ? "timed_out"
      : "ok";
  const unavailableReason = summaries.find((summary) => summary.status === "unavailable")?.unavailableReason;
  const configurationIssues = [...new Set(summaries.flatMap((summary) => summary.configurationIssues))];
  const stale = summaries.some((summary) => summary.stale);
  const mergedStatus: DiagnosticStatus = status === "unavailable" || unavailableReason ? "unavailable" : status;
  const bySeverity = {
  ...
  return {
    status: mergedStatus,
    ...summarizeConclusion(mergedStatus, items.length, stale, configurationIssues.length),
    timedOut: summaries.some((summary) => summary.timedOut),
    stale,
    ...(unavailableReason ? { unavailableReason } : {}),
    configurationIssues,
    sourceRevision: maxSourceRevision(summaries),
    total: items.length,
    bySeverity,
    items,
...
function filterDiagnosticSummary(
  ...
  return {
    ...summary,
    total: items.length,
    ...summarizeConclusion(summary.status, items.length, summary.stale, summary.configurationIssues.length),
    bySeverity: {
      error: items.filter((item) => item.severity === "error").length,
      warning: items.filter((item) => item.severity === "warning").length,
      information: items.filter((item) => item.severity === "information").length,
...
function maxSourceRevision(summaries: Array<Awaited<ReturnType<WorkspaceCommandService["diagnostics"]>>>): number | undefined {
  const revisions = summaries.flatMap((summary) => (summary.sourceRevision === undefined ? [] : [summary.sourceRevision]));
