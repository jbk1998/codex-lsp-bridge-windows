...
export function canonicalizeFileUri(uri: string): string {
  ...
  const normalizedDrive = uri.replace(/^file:\/\/\/([a-z])(?:%3A|:)/i, (_match, drive: string) => `file:///${drive.toUpperCase()}:`);
  return pathToFileURL(canonicalizeFilePath(fileURLToPath(normalizedDrive))).toString();
}
